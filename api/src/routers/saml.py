"""SAML 2.0 SSO – SP-initiated Login über einen beliebigen IdP (z.B.
FortiAuthenticator). Portiert aus dem ids-Projekt, auf admin/viewer getrimmt.
...
def _app_cfg(request: Request) -> Config:
...
def _prepare_request(request: Request, form_data: dict | None = None,
                     acs_url: str | None = None) -> dict:
    """Baut das request_data-Dict für python3-saml. acs_url überschreibt
    Schema/Host/Port, um Validierungsfehler bei Port-Mapping / fehlendem
    X-Forwarded-Proto zu vermeiden."""
    if acs_url:
...
async def _upsert_saml_user(pool: asyncpg.Pool, username: str, email: str,
                            display_name: str, default_role: str) -> asyncpg.Record:
    ...
    async with pool.acquire() as conn:
        ...
        row = await conn.fetchrow(
            """
            INSERT INTO users (username, email, display_name, role, source, active)
            ...
            RETURNING *
            """,
            username, email or None, display_name or username, default_role or "viewer",
        )
    return row
...
async def saml_login(request: Request,
                     pool: asyncpg.Pool = Depends(get_pool)) -> RedirectResponse:
    try:
...
async def saml_acs(request: Request, pool: asyncpg.Pool = Depends(get_pool),
                   app_cfg: Config = Depends(_app_cfg)) -> RedirectResponse:
    try:
        from onelogin.saml2.auth import OneLogin_Saml2_Auth
    except ImportError:
        raise HTTPException(500, "python3-saml nicht installiert.")
    ...
    user = await _upsert_saml_user(pool, username, email, display_name,
                                   cfg.get("default_role", "viewer"))

    token = create_token(app_cfg.secret_key, str(user["id"]),
                         user["username"], user["role"])
    return RedirectResponse(f"/?saml_token={token}", status_code=302)


@router.get("/metadata", summary="SP-Metadata XML für den IdP")
async def sp_metadata(request: Request,
                      pool: asyncpg.Pool = Depends(get_pool)) -> Response:
    try:
...
async def saml_sls(request: Request,
    ...
    return RedirectResponse(url or "/", status_code=302)
